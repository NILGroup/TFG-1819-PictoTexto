...
A lo largo de este capitulo expondremos la aplicación, Pict2Text que hemos desarrollado para este proyecto, la cual podemos ver en la Figura \ref{fig:pict2Text}. Pict2Text es una aplicación que permite la traducción de pictogramas a lenguaje natural, aunque además posee un buscador de pictogramas, el cual sirve de entrada para los pictogramas, que permite hacer búsquedas de los pictogramas de ARAASAC que tienen asociada una determinada palabra, el cual podemos ver en la Figura \ref{fig:buscadorPictogramas}. Hablaremos sobre los principios de arquitectura en los cuales nos hemos basado. 
\figura{Bitmap/Arquitectura/buscadorPictogramas}{width=8cm}{fig:buscadorPictogramas}{Buscador de pictogramas de Pict2Text }
Y detallaremos los diferentes servicios web implementados para la parte Back-End, los cuales hemos decidido clasificar en dos grupos: Servicios Web para la Generación de Frases, donde hablaremos de los servicios web implementados para la generación de lenguaje natural, y los Servicios Web para la Gestión de Pictogramas donde están englobados aquellos servicios web necesarios para el uso de pictogramas. 

\figura{Bitmap/Arquitectura/ServiciosWebDiagrama}{height=14cm, width=13cm}{fig:ServiciosWebDiagrama}{Pict2Text visión general }
Por ultimo hablaremos sobre la aplicación Front-End desarrollada con el framework Angular, describiendo los diferentes componentes que la forman y la relación que guardan con la parte Back-End. En la Figura \ref{fig:ServiciosWebDiagrama}, podemos ver un diagrama en el que se muestran las partes Back y Fron-End y su relación.
...
\begin{itemize}
\item Creación del objeto sujeto: gracias a la función \textbf{createNounPhrase()} de la librería SimpleNLG, creamos el sintagma nominal que sera el objecto contenedor de todas las palabras que formaran el sujeto.
 
\item Selección del determinante: En este proceso se establecerá el determinante del sujeto, utilizando la función \textbf{setDeterminer(``determinante'')} de SimpleNLG, sobre el objecto creado anteriormente. En caso de que la lista de palabras que formaran el sujeto, no contenga un pronombre o un determinante, se añade el determinante por defecto(``el''). En caso contrario la se añadirá el determinante o pronombre, en lugar del determinante de por defecto.
...
El servicio proxy de la aplicación Angular sirve para abstraer las diferentes llamadas HTTP del resto de la aplicación, este componente es necesario ya que sirve de punto de acceso a los Servicios Web explicados en la seccion Back-End \ref{cap5:sec:backEnd}, de este capitulo. Los métodos se encargan de emitir las peticiones HTTP (GET o POST), esperar la respuesta del servicio y devolverla al servicio de aplicación.
