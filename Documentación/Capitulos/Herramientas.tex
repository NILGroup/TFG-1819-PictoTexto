...
\begin{itemize}
    \item SearchText\footnote{\url{https://beta.arasaac.org/developers/api/pictograms/search/searchText}}: Dada una palabra devuelve todos los pictogramas asociados a ella.Por ejemplo, dada la palabra ``perro'' nos devuelve el JSON, mostrado en la Figura \ref{fig:jsonPerro}. Dicho JSON contiene toda la información relevante a dicha palabra, tipo de palabra, significado, sinónimos, plurales y keyword o palabra por la que se traduce, además de proporcionarnos sus diferentes ``idPictogram'' con los que obtendremos los pictogramas.
